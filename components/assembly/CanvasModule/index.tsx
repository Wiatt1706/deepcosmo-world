...
const CanvasBoard: React.FC<CanvasModuleProps> = observer((props) => {
  ...
  useEffect(() => {
    ...
    canvasStore.setImgSrc(clipImage());
  ...
  const clipImage = () => {
    ...
    const width = props.showSize.width * dpr; // 指定宽度
    const height = props.showSize.height * dpr; // 指定高度

    // 计算正中间区域的起始坐标
    const centerX = canvasWidth / 2 - width / 2; // 中心X坐标减去一半的宽度
    const centerY = canvasHeight / 2 - width / 2; // 中心Y坐标减去一半的高度

    // 获取指定区域的像素数据
    const imageData = context.getImageData(centerX, centerY, width, height);
